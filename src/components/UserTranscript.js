...
{
    const WORDS = JSON.parse(JSON.stringify(words)); // pls ignore this way of cloning
        ...
        <div className="row">
            ...
            <div className={`col-md-${index === 1 ? '10' : '9'} text`}>
                {WORDS.map((w,i) => {
                    ...
                    return <span key={ i} className={`${((currentTime >= w.startTime) && (currentTime < w.endTime)) ? 'active' : ''} ${w.isSearchText ? 'search-text' : ''} word`} onClick={() => wordClick(w)}>{w.word} </span>
                })}
            </div>
        </div>
    </div>
